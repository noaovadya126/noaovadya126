...
def login(env: str, username: str, password: str) -> str:
    ...
    """
    LOGIN_URL = "ums/v2/users/login"
    url = f"{url_base(env=env)}{LOGIN_URL}"

    headers = {"content-type": "application/json"}
    credentials = {"username": username, "password": password}

    resp = requests.post(url=url, headers=headers, json=credentials)
...
# API URLs
url = (
    "https://api.fda.neteerahealthbridge.neteera.com/device/v2/devices/stats/"
    "continuously-disconnected/count?continuousDisconnectionSeconds=86400"
)
url2 = (
    "https://api.fda.neteerahealthbridge.neteera.com/organization/v2/tenants/"
    "30e13fa2-c7bb-4d36-8476-0647891ca95d/sub-tenants?limit=10000000"
)
...
df["Disconnected"] = df[
    ["disconnectedAssigned", "disconnectedUnassigned"]
].sum(axis=1)
...
df = df[
    [
        "Group", "Tenant Name", "Total", "Disconnected",
        "connectedAssigned", "disconnectedAssigned",
        "connectedUnassigned", "disconnectedUnassigned"
    ]
]
